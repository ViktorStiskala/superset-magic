...
- **`cache.rs` knows nothing about hooks.** Its keyed-store functions (`load`/`write`/`gc`, and the `conclude`/`conclusions`/`gc` verbs built on them) are gate-exclusive – `pre_tool_use.rs` is their only caller. The untrusted-data framing helpers, `envelope` and `Budget`, are shared more widely: `hook/subagent_stop.rs` and both `checklist/render.rs` and `checklist/verbs.rs` call into them too, and `ledger.rs` may reuse the hashing.
